...
class WindowsDiagnosticTests(unittest.TestCase):
    ...
    def test_install_argv_keeps_frozen_hermes_ref_and_private_tool_paths(self):
        ...
        self.assertNotIn("NAN_API_KEY", install[1])
        # The installer authenticates its GitHub API calls; the provider token never reaches it
        # and the workflow token never reaches the probe or the harness.
        self.assertEqual(install[1].get("GITHUB_TOKEN"), "secret")
        probe = next(call for call in calls if "-Stage" in call[0])
        self.assertNotIn("GITHUB_TOKEN", probe[1])
        self.assertNotIn("NAN_API_KEY", probe[1])
    ...
    def test_installer_gets_the_token_and_the_harness_environment_does_not(self):
        ...
        def fake(argv, cwd, env, timeout):
            if any("install-harness.ps1" in str(argument) for argument in argv):
                envs["install"] = dict(env)
                return (0, "exit")
            if "-Stage" in argv:
                envs.setdefault("probe", dict(env))
                self.write_probe_success(argv, env)
            return (0, "exit")
        with tempfile.TemporaryDirectory() as tmp, patch.object(diagnostic, "resolver_module", return_value=Resolver()), \
             patch.object(diagnostic.shutil, "which", return_value="x"), patch.object(diagnostic, "run_bounded", side_effect=fake), \
             patch.dict(diagnostic.os.environ, {"GITHUB_TOKEN": "token-value"}):
            self.binaries(args, Path(tmp)); diagnostic.collect(args, ["codex"], Path(tmp))
        self.assertEqual(envs["install"].get("GITHUB_TOKEN"), "token-value")
        self.assertNotIn("GITHUB_TOKEN", envs["probe"])
