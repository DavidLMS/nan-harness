...
def collect(args, harnesses, output):
    ...
    for harness in selected:
        ...
        while True:
            ...
            # an anonymous shared runner hits the rate limit (HTTP 403). The token is added to
            # the installer child only, never to the probe or harness environment.
            install_env = dict(env)
            if os.environ.get("GITHUB_TOKEN"):
                install_env["GITHUB_TOKEN"] = os.environ["GITHUB_TOKEN"]
            code, reason = run_bounded(command, cell, install_env, child_timeout)
            if code == 0:
                break
