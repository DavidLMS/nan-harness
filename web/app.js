...
const translations = {
  en: {
    ...
    heroLede: "Use NaN's open models from the coding agents you already know. Run them through nan-harness or configure them for NaN. No new interface to learn.",
    whatIs: 'WHAT IS NAN-HARNESS',
    oneLocal: 'The model moves.',
    everyAgent: 'Your workflow stays.',
    whatText: 'NaN gives you the models. Your agent gives you the loop. nan-harness connects them in two ways: it can run the agent with a managed NaN route, or add NaN to the agent\'s own provider configuration. Either way, you keep the original client and workflows.',
    howItWorks: 'HOW IT WORKS',
    workflowLabel: 'TWO WAYS TO USE IT',
    discovering: '→ discovering harness',
    startingRoute: '→ starting local route',
    ready: '→ harness ready',
    workflowHeading: 'Run it through nan-harness.',
    workflowSubheading: 'Or configure it for NaN.',
    workflowText: 'Use <code>nan &lt;harness&gt;</code> when you want nan-harness to check compatibility, prepare the route and run the agent. Use <code>nan config &lt;harness&gt;</code> when you want a supported agent configured for NaN and plan to start it with its own command. Configuration never launches the agent.',
    workflowMode: 'managed launch / native setup',
    managedLaunchResult: '→ nan-harness starts and manages OpenCode',
    nativeSetupResult: '→ writes OpenCode\'s native NaN configuration',
    directLaunchResult: '→ OpenCode starts directly with NaN',
    ...
    docsSections: [
      ...
      ['harnesses', 'HARNESSES', [
        ['p', 'There are two independent ways to use nan-harness. <code>nan &lt;agent&gt;</code> starts the agent under nan-harness management. <code>nan config &lt;agent&gt;</code> configures a supported agent for NaN so you can start it later with its own command.'],
        ['table', ['Managed launch', 'Agent', 'Native setup'], [
          ['nan claude', harnessLink('Claude Code', 'claude'), 'needs nan-harness'],
          ['nan codex', harnessLink('Codex', 'codex'), 'needs nan-harness'],
          ['nan opencode', harnessLink('OpenCode', 'opencode'), 'available'],
          ['nan hermes', harnessLink('Hermes Agent', 'hermes'), 'available'],
          ['nan pi', harnessLink('Pi', 'pi'), 'available'],
          ['nan prime-agent', harnessLink('Prime Agent', 'prime'), 'available'],
          ['nan dsh', harnessLink('DeepSeek Harness', 'deepseek'), 'available'],
          ['nan openclaw', harnessLink('OpenClaw', 'openclaw'), 'available'],
          ['nan cline', harnessLink('Cline', 'cline'), 'available'],
          ['nan qwen', harnessLink('Qwen Code', 'qwen'), 'available'],
          ['nan kimi', harnessLink('Kimi Code', 'kimi'), 'available'],
          ['nan aider', harnessLink('Aider', 'aider'), 'available'],
          ['nan goose', harnessLink('Goose', 'goose'), 'available'],
          ['nan fx', harnessLink('fx', 'fx'), 'needs nan-harness']
        ]],
        ['h3', 'Managed launch'],
        ['p', 'Run <code>nan &lt;agent&gt;</code> when you want nan-harness to check the installed version, discover your NaN models, prepare any required bridge and supervise the agent process. If the agent is missing, nan-harness can offer to install it.'],
        ['code', 'nan opencode'],
        ['h3', 'Native setup'],
        ['p', 'Run <code>nan config &lt;agent&gt;</code> when you want to add NaN to a supported agent\'s own provider configuration. This command only configures the agent. Start it afterwards with its usual executable:'],
    ...
    docsLede: "This documentation explains how to use NaN's models from your coding agents: launch them through nan-harness, or configure supported agents and run them directly.",
    ...
    gettingStartedText: 'Choose the workflow that fits you. Let nan-harness launch and supervise your agent, or use native setup and keep starting the agent with its own command.',
    examplesText: 'Both workflows preserve the agent you already know. The difference is whether nan-harness manages each launch or prepares the agent itself for NaN.',
    ...
    faqs: [
      ...
      ['Does it replace my agent?', 'No. You keep the original agent, profile and workflows. nan-harness either manages the launch and NaN connection, or writes the agent\'s native provider configuration so you can run it directly.'],
      ['Do I have to start agents with nan-harness?', 'No. For agents with native setup, run <code>nan config &lt;agent&gt;</code> once and then use the agent\'s normal command. Claude Code, Codex and fx need nan-harness running because their NaN connection uses a local bridge or gateway.'],
      ['How are models discovered?', 'Managed launches read the live NaN catalog. Native setup writes a catalog snapshot into the agent; run <code>nan config &lt;agent&gt; --refresh</code> when your available models change.'],
  ...
  es: {
    ...
    heroLede: 'Usa los modelos abiertos de NaN con los agentes de código que ya conoces. Arráncalos mediante nan-harness o configúralos para NaN. Sin aprender otra interfaz.',
    whatIs: 'QUÉ ES NAN-HARNESS',
    oneLocal: 'El modelo se mueve.',
    everyAgent: 'Tu flujo se mantiene.',
    whatText: 'NaN te da los modelos. Tu agente te da el loop. nan-harness los conecta de dos formas: puede arrancar el agente con una conexión gestionada a NaN o añadir NaN a la configuración de proveedores del propio agente. En ambos casos conservas el cliente y sus flujos originales.',
    howItWorks: 'CÓMO FUNCIONA',
    workflowLabel: 'DOS FORMAS DE USARLO',
    discovering: '→ descubriendo harness',
    startingRoute: '→ iniciando ruta local',
    ready: '→ harness listo',
    workflowHeading: 'Arráncalo con nan-harness.',
    workflowSubheading: 'O configúralo para NaN.',
    workflowText: 'Usa <code>nan &lt;harness&gt;</code> cuando quieras que nan-harness compruebe la compatibilidad, prepare la conexión y arranque el agente. Usa <code>nan config &lt;harness&gt;</code> cuando quieras configurar un agente compatible para NaN y abrirlo después con su propio comando. Configurar nunca arranca el agente.',
    workflowMode: 'arranque gestionado / configuración nativa',
    managedLaunchResult: '→ nan-harness arranca y gestiona OpenCode',
    nativeSetupResult: '→ escribe la configuración nativa de NaN en OpenCode',
    directLaunchResult: '→ OpenCode arranca directamente con NaN',
    ...
    docsSections: [
      ...
      ['harnesses', 'AGENTES', [
        ['p', 'Hay dos formas independientes de usar nan-harness. <code>nan &lt;agente&gt;</code> arranca el agente bajo la gestión de nan-harness. <code>nan config &lt;agente&gt;</code> configura un agente compatible para NaN y permite abrirlo después con su propio comando.'],
        ['table', ['Arranque gestionado', 'Agente', 'Configuración nativa'], [
          ['nan claude', harnessLink('Claude Code', 'claude'), 'necesita nan-harness'],
          ['nan codex', harnessLink('Codex', 'codex'), 'necesita nan-harness'],
          ['nan opencode', harnessLink('OpenCode', 'opencode'), 'disponible'],
          ['nan hermes', harnessLink('Hermes Agent', 'hermes'), 'disponible'],
          ['nan pi', harnessLink('Pi', 'pi'), 'disponible'],
          ['nan prime-agent', harnessLink('Prime Agent', 'prime'), 'disponible'],
          ['nan dsh', harnessLink('DeepSeek Harness', 'deepseek'), 'disponible'],
          ['nan openclaw', harnessLink('OpenClaw', 'openclaw'), 'disponible'],
          ['nan cline', harnessLink('Cline', 'cline'), 'disponible'],
          ['nan qwen', harnessLink('Qwen Code', 'qwen'), 'disponible'],
          ['nan kimi', harnessLink('Kimi Code', 'kimi'), 'disponible'],
          ['nan aider', harnessLink('Aider', 'aider'), 'disponible'],
          ['nan goose', harnessLink('Goose', 'goose'), 'disponible'],
          ['nan fx', harnessLink('fx', 'fx'), 'necesita nan-harness']
        ]],
        ['h3', 'Arranque gestionado'],
        ['p', 'Usa <code>nan &lt;agente&gt;</code> cuando quieras que nan-harness compruebe la versión instalada, descubra tus modelos de NaN, prepare los bridges necesarios y supervise el proceso. Si falta el agente, nan-harness puede ofrecerte instalarlo.'],
        ['code', 'nan opencode'],
        ['h3', 'Configuración nativa'],
        ['p', 'Usa <code>nan config &lt;agente&gt;</code> cuando quieras añadir NaN a la configuración de proveedores del propio agente. Este comando solo configura. Después, arranca el agente con su ejecutable habitual:'],
        ['codes', ['nan config opencode', 'opencode']],
      ...
      ['options', 'OPCIONES', [
        ['h3', 'Opciones del arranque gestionado'],
        ...
        ['h3', 'Comandos de configuración nativa'],
    ...
    docsLede: 'Esta documentación explica cómo usar los modelos de NaN desde tus agentes de código: puedes arrancarlos mediante nan-harness o configurar los agentes compatibles y ejecutarlos directamente.',
    ...
    gettingStartedText: 'Elige la forma de uso que te encaje. Deja que nan-harness arranque y supervise el agente, o usa la configuración nativa y sigue abriéndolo con su propio comando.',
    examplesText: 'Las dos opciones conservan el agente que ya conoces. La diferencia es si nan-harness gestiona cada arranque o prepara el propio agente para NaN.',
    ...
    faqs: [
      ...
      ['¿Sustituye a mi agente?', 'No. Conservas el agente original, su perfil y sus flujos. nan-harness puede gestionar el arranque y la conexión con NaN, o escribir la configuración nativa del proveedor para que ejecutes el agente directamente.'],
      ['¿Tengo que arrancar los agentes con nan-harness?', 'No. En los agentes con configuración nativa, ejecuta <code>nan config &lt;agente&gt;</code> y después usa el comando normal del agente. Claude Code, Codex y fx necesitan nan-harness en ejecución porque su conexión con NaN usa un bridge o gateway local.'],
      ['¿Cómo se descubren los modelos?', 'Los arranques gestionados leen el catálogo actual de NaN. La configuración nativa escribe una copia del catálogo en el agente; ejecuta <code>nan config &lt;agente&gt; --refresh</code> cuando cambien tus modelos disponibles.'],
