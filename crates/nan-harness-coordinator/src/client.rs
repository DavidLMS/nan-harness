...
impl CoordinatorClient {
    ...
    async fn connect_or_start(&self) -> Result<(TcpStream, Receipt), CoordinatorError> {
        self.connect_or_spawn(spawn_daemon).await
    }

    /// Connects to a running coordinator and starts one with `spawn` when none answers.
    ///
    /// Production always passes [`spawn_daemon`]; the parameter exists so tests can
    /// publish a synthetic daemon instead of launching a real process.
    async fn connect_or_spawn(
        &self,
        spawn: impl FnOnce(),
    ) -> Result<(TcpStream, Receipt), CoordinatorError> {
        if self
            .retry_probe_at
            .lock()
