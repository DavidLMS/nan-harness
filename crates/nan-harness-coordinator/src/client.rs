...
const CONTROL_ACK_BUDGET: Duration = Duration::from_millis(500);
...
impl RequestLease {
    pub async fn headers_received(&mut self, elapsed: Duration) {
        ...
        if !matches!(
            tokio::time::timeout(CONTROL_ACK_BUDGET, write_frame(stream, &message)).await,
            Ok(Ok(()))
        ) {
            self.stream = None;
        }
    ...
    pub async fn observe(
        ...
        let Ok(response) = tokio::time::timeout(CONTROL_ACK_BUDGET, async {
            ...
            read_frame::<ServerMessage>(stream).await
        })
        .await
        else {
            self.stream = None;
...
mod lease_tests {
    ...
    async fn start_granted_lease(listener: TcpListener) -> (RequestLease, TcpStream) {
        ...
        let socket = TcpSocket::new_v4().expect("client socket should be created");
        socket
            .set_send_buffer_size(1)
            .expect("client send buffer should be reduced");
    ...
    async fn progress_write_failure_closes_the_lease() {
        ...
        drop(server);
        tokio::time::sleep(Duration::from_millis(10)).await;
        let stream = lease.stream.as_mut().expect("lease should stay connected");
        let mut write_failed = false;
        for _ in 0..128 {
            let fill = vec![0_u8; 64 * 1024];
            if stream.write_all(&fill).await.is_err() {
                write_failed = true;
                break;
            }
        }
        assert!(write_failed, "disconnected peer should reject writes");
        assert!(
            stream.write(&[0_u8]).await.is_err(),
            "disconnected peer should keep rejecting writes"
        );
