...
fn process_matches_maps_exit_statuses_and_missing_commands() {
    // Use an existing executable for exit-status contracts. Executing freshly
    // written scripts races with concurrent forks on Linux (ETXTBSY).
    assert!(matches!(
        process_matches("/bin/sh", &["-c", "exit 0"]),
        Ok(true)
    ));
    assert!(matches!(
        process_matches("/bin/sh", &["-c", "exit 1"]),
        Ok(false)
    ));

    assert!(matches!(
        process_matches("/bin/sh", &["-c", "exit 2"]),
...
fn run_launcher_maps_success_nonzero_and_missing_commands() {
    assert!(run_launcher("/bin/sh", &["-c".as_ref(), "exit 0".as_ref()]).is_ok());

    let result = run_launcher("/bin/sh", &["-c".as_ref(), "exit 7".as_ref()]);
    assert!(
        matches!(result, Err(ClaudeDesktopError::LaunchFailed(Some(7)))),
...
fn terminate_matches_accepts_documented_statuses_only() {
    for code in [0, 1, 128] {
        let result = terminate_matches("/bin/sh", &["-c", &format!("exit {code}")]);
        assert!(result.is_ok(), "synthetic exit {code}: {result:?}");
    }

    assert!(matches!(
        terminate_matches("/bin/sh", &["-c", "exit 2"]),
