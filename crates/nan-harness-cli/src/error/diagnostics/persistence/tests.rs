...
fn persistence_diagnostics_cover_directory_io_conflicts_and_configuration() {
    for (error, expected, injected_values) in directory_io_cases()
        .into_iter()
        .chain(configuration_cases())
    {
        assert_diagnostic(&error, &expected, injected_values);
    }
}

fn directory_io_cases() -> Vec<(PersistenceError, Diagnostic, &'static [&'static str])> {
    let cases: Vec<(PersistenceError, Diagnostic, &[&str])> = vec![
        (
        ...
        (
            ...
            io_diagnostic(DiagnosticOperation::WriteConfiguration, IoErrorKind::Other),
            &[FAKE_PATH, FAKE_SOURCE][..],
        ),
        (
            ...
            io_diagnostic(DiagnosticOperation::RemoveConfiguration, IoErrorKind::Other),
            &[FAKE_PATH, FAKE_SOURCE][..],
        ),
    ];
...
fn configuration_cases() -> Vec<(PersistenceError, Diagnostic, &'static [&'static str])> {
    configuration_path_cases()
        .into_iter()
        .chain(configuration_document_cases())
        .collect()
}

fn configuration_path_cases() -> Vec<(PersistenceError, Diagnostic, &'static [&'static str])> {
    let invalid_utf8 = String::from_utf8(vec![0xff]).expect_err("fixture must be invalid UTF-8");
...
fn configuration_document_cases() -> Vec<(PersistenceError, Diagnostic, &'static [&'static str])> {
    let invalid_json = serde_json::from_str::<serde_json::Value>("{").expect_err("invalid JSON");
...
fn persistence_diagnostics_cover_model_discovery_secrets_and_state_documents() {
    for (error, expected, injected_values) in model_discovery_cases()
        .into_iter()
        .chain(state_document_cases())
    {
        assert_diagnostic(&error, &expected, injected_values);
    }
}

fn model_discovery_cases() -> Vec<(PersistenceError, Diagnostic, &'static [&'static str])> {
    let invalid_json = serde_json::from_str::<serde_json::Value>("{").expect_err("invalid JSON");
...
fn state_document_cases() -> Vec<(PersistenceError, Diagnostic, &'static [&'static str])> {
    let cases: Vec<(PersistenceError, Diagnostic, &[&str])> = vec![
        (
